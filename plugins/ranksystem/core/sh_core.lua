...
function canPromote(promoter,who,rank)
    if promoter:GetPlayer():IsAdmin() then //jest admin  to moze
        //print("keine Verwaltung")
        return true 
    end
    if !(promoter:GetFaction()==who:GetFaction()) then //inna  frakcja nie moze
        //print("nie ta frakcja")
        promoter:GetPlayer():Notify("Sie können niemanden außerhalb Ihrer Fraktion befördern.")
        return false
    end
    ...
    if isstring(rank) then 
        ...
        promoter:GetPlayer():Notify("Es gibt keinen solchen Rang.")
        return false
    end
    local pRank = promoter:GetRank()
    if pRank<=rank then //jak ktos chce awansowac typa wyzej od sb nie moze
        //print("za wysoko")
        promoter:GetPlayer():Notify("Sie können niemanden befördern, der höher ist als Sie selbst oder den gleichen Rang hat..")
        return false
    end
