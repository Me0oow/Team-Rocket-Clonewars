...
PLUGIN.description = "A system for simple cutscenes."
PLUGIN.fadeDelay = 0.5
PLUGIN.cutscenes = PLUGIN.cutscenes or {}

ix.config.Add("playIntroScene", true, "Whether or not to play the intro scene for new chars (just in case)", nil, {
...
function PLUGIN:OnCharacterCreated(client)
    if ix.config.Get("playIntroScene", true) and not client:IsAdmin() then
        PLUGIN:runCutscene(client, "introscene", true)
    end
end