...
function PANEL:BuildSlots()
	...
	for x = 1, self.gridW do
		...
		for y = 1, self.gridH do
			...
			surface.DrawRect(2, 2, width - 4, height - 4)  --сюда добавить картинки


			--surface.SetDrawColor(255, 255, 255, 25)
	        --surface.SetMaterial(Material("karman/metro/corner.png"))
	        --surface.DrawTexturedRect(47,  47 ,60  , 60 )
	...
	if (self.slots[x] and self.slots[x][y]) then
		...
		if (itemTable.exRender) then
			...
			panel.ExtraPaint = function(this, panelX, panelY)
				...
				if (exIcon) then
					surface.SetMaterial("karman/background.png")
...
hook.Add("CreateMenuButtons", "ixInventory", function(tabs)
	...
	local SCREEN_SCALE = ScrH() / 1080
				local client = LocalPlayer()
			local character = client:GetCharacter()
			local modelFOV = (ScrW() > ScrH() * 1.8) and 38 or 34
...
local gradientD = ix.util.GetMaterial("vgui/gradient-d")
	...
	tabs["inv"] = {
		...
		Create = function(info, container)
			...
			/*
				local equipParent = canvas:Add("DPanel")
			equipParent:SetPos(0, 0)
			equipParent:SetSize(364 * SCREEN_SCALE, ScrH()-189)
			equipParent.bNoBackgroundBlur = true



						-- Model
			local model = equipParent:Add("ixModelPanel")
			model:SetSize(200 * SCREEN_SCALE, 900 * SCREEN_SCALE)
			model:SetPos(70 * SCREEN_SCALE, 5 * SCREEN_SCALE)
		...
		--	model.LayoutEntity = function() end
			model.Think = function()
				model:SetVisible(IsValid(ix.gui.menu) and !ix.gui.menu.bClosing)
   				...
   				local weight = equipParent:Add("DPanel")
					weight:SetPos(-230, h + 825)
					weight:SetSize(1500, 1500)
...
*/
						...
						--surface.DrawOutlinedRect(0, 0, w, h)
    						...
    						surface.SetDrawColor(105, 105, 105, 85)
						surface.DrawRect(280, -560, w/5.2, h -930)

						    						surface.SetDrawColor(255, 255, 255, 85)
						surface.DrawRect(280, -560, math.Clamp(kyshat, 0, 100)*2.9, h -930)

        //surface.SetDrawColor( 255, 255, 255, 255 )
    // surface.SetMaterial( hunger )
    // surface.DrawTexturedRect( 250, -1, 16, 16 )
		...
		local inventory = LocalPlayer():GetCharacter():GetInventory()
			...
			local weight = panel:Add("DPanel")
				...
				local hungerMat = Material( "karman/hud/hunger.png" )
				local hungerBar = panel:Add("DPanel")
					hungerBar:SetTooltip("Hunger")
					hungerBar:SetPos(5, h + 20)
					hungerBar:SetSize(w, 24)
					hungerBar.Paint = function(self, w, h)
						surface.SetDrawColor(35, 35, 35, 85)
						surface.DrawRect(1, 1, w, h)
					...
					local bar = hungerBar:Add("DPanel")
						bar:SetSize(w, 24)
						bar.Paint = function(self, w, h)
							surface.SetDrawColor(Color(122,122,122))
							surface.DrawRect(0, 4, math.min((w / 100) * hunger, w), h)
						end
					local barT = hungerBar:Add("DLabel")
						barT:SetSize(w, 24)
						barT:SetContentAlignment(2)
						barT:SetFont(1201)
						barT.Paint = function(self, w, h)
							surface.SetDrawColor(255, 255, 255, 255)
							surface.SetMaterial( hungerMat )
							surface.DrawTexturedRect( w*0.5 - h * 1.25, h*0.35, h * 0.5, h * 0.5 )
						end
						barT.Think = function()
							hunger = character:GetHunger()
							barT:SetText(math.Round(hunger, 2).."%")
						end
