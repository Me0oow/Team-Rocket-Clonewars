...
function PANEL:Init()
	...
	if (logo and !logo:IsError()) then
		...
		logoImage:SetPos(halfWidth - logoImage:GetWide() * 0.5, halfPadding / 10)
	...
	local createButton = self:Add("ixMenuButton")
	createButton:SetText("create")
	createButton:SetTextColor(Color(47,87,120))
	createButton:SetContentAlignment(5)
	createButton:SizeToContents()
	createButton:SetSize( ScrW() / 4.315 * 2, ScrH() / 32.5 * 2)
	createButton:SetPos( ScrW() / 3.72, ScrH() / 1.84 )
	...
	self.loadButton = self:Add("ixMenuButton")
	self.loadButton:SetText("load")
	self.loadButton:SetTextColor(Color(47,87,120))
	self.loadButton:SetContentAlignment(5)
	self.loadButton:SizeToContents()
			self.loadButton:SetSize( ScrW() / 4.315 * 2, ScrH() / 32.5 * 2 )
	self.loadButton:SetPos( ScrW() / 3.72, ScrH() / 1.59 )	
	...
	if (extraURL != "" and extraText != "") then
		...
		local extraButton = self:Add("ixMenuButton")
		extraButton:SetText(extraText, true)
			extraButton:SetTextColor(Color(47,87,120))
		extraButton:SetContentAlignment(5)
		extraButton:SizeToContents()
		extraButton:SetSize( ScrW() / 4.315 * 2, ScrH() / 32.5 * 2 )
	    extraButton:SetPos( ScrW() / 3.72, ScrH() / 1.4 )
	...
	-- leave/return button
	self.returnButton = self:Add("ixMenuButton")
	self.returnButton:SetContentAlignment(5)
	self:UpdateReturnButton()
			self.returnButton:SetSize( ScrW() / 4.315 * 2, ScrH() / 32.5 * 2 )
	self.returnButton:SetPos( ScrW() / 3.72, ScrH() / 1.25 )
...
function PANEL:UpdateReturnButton(bValue)
	...
	self.returnButton:SetText(bValue and "return" or "leave")
		self.returnButton:SetTextColor(Color(47,87,120))
...
function PANEL:Paint(width, height)
	...
	surface.SetMaterial(Material("berry/background.png"))
...
function PANEL:Close(bFromMenu)
	...
	local fadeOutTime = animationTime * 1
