...
function ITEM:GetDescription()
	if (self.entity) then
		return L(self.description .. "\n \n Rüstungsstand:" .. math.floor(self:GetData("Durability", 100)))
	else
        return L(self.description .. "\n \n Rüstungsstand:" .. math.floor(self:GetData("Durability", 100)) .. "\n \nWiederstand: \n  Schusssicher: " .. (self.damage[1]) .. "\n  Panzerung: " .. (self.damage[2]) .. "\n  Stromwiederstand: " .. (self.damage[3]) .. "\n  Feuerwiederstand: " .. (self.damage[4]) .. "\n  Strahlungsschutz: " .. (self.damage[5]) .. "\n  Chemischer Schutz: " .. (self.damage[6]) .. "\n  Schock: " .. (self.damage[7]))
	end
end
...
ITEM.functions.repair = {
	...
	OnRun = function(item)
		...
		local number = 0
		if (character:GetAttribute("reparieren") >= 5) then
			for k, v in pairs(items) do
				if (v:GetName() == "Metall" and number < 2) then
					number = number + 1
					v:Remove()
				end
			end
			if (number == 2) then
				item:SetData("Durability", 5000)
				number = 0
			else
				client:Notify("Du brauchst zwei weitere Metal Teile dafür!")
			end
		else
			client:Notify("Du brauchst ein höheres Level an Reperatur Fähgikeiten")
		end
		return false
	end
}
