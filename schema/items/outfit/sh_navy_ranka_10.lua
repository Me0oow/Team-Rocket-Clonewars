ITEM.name = "Rank Armor 10";
ITEM.model = "models/cs574/objects/ammo_box.mdl"
ITEM.price = 0;
ITEM.description = "Navy Kleidung."
ITEM.outfitCategory = "rank armor";
ITEM.bodyGroups = {
	["rank armor"] = 10,

}

ITEM.category = "Navy-Kleidung"
ITEM.width = 1
ITEM.height = 1
...
if (CLIENT) then
	...
	function ITEM:PopulateTooltip(tooltip)
		local wear = tooltip:AddRowAfter("rarity")
		wear:SetText(L("wearSlot", L(self.outfitCategory)))
	end
end
	...
	if (character:GetData("oldGroups" .. self.outfitCategory)) then
		...
		if (self.bodyGroups) then
			...
			for k, value in pairs(self.bodyGroups) do
				local index = k
				if isstring(k) then
					index = self.player:FindBodygroupByName(k)
				end
	...
	OnRun = function(item)
		...
		if (item.bodyGroups) then
			...
			for k, value in pairs(item.bodyGroups) do
				local index = k
				if isstring(k) then
					index = item.player:FindBodygroupByName(k)
				end
